...
def analyze_training_metrics(metrics, save_dir="results"):
    ...
    try:
        ...
        active_cars = metrics.get("active_cars", [])
        ...
        summary = {
            ...
            "active_cars": {
                "mean": float(np.mean(active_cars)) if active_cars else None,
                "min": float(np.min(active_cars)) if active_cars else None,
                "max": float(np.max(active_cars)) if active_cars else None
            },
        ...
        # Plot waiting times, throughput and car counts
        if waiting_times and throughputs:
            num_subplots = 3 if active_cars else 2
            fig, axes = plt.subplots(num_subplots, 1, figsize=(12, 5 * num_subplots))
            
            axes[0].plot(waiting_times, color='#e67e22', linewidth=1.5)
            axes[0].set_xlabel('Episode')
            axes[0].set_ylabel('Average Waiting Time')
            axes[0].set_title('Average Waiting Time Per Episode', fontweight='bold')
            axes[0].grid(True, linestyle='--', alpha=0.3)
            
            axes[1].plot(throughputs, color='#27ae60', linewidth=1.5)
            axes[1].set_xlabel('Episode')
            axes[1].set_ylabel('Average Throughput')
            axes[1].set_title('Average Throughput Per Episode', fontweight='bold')
            axes[1].grid(True, linestyle='--', alpha=0.3)
            
            if active_cars:
                axes[2].plot(active_cars, color='#3498db', linewidth=1.5)
                axes[2].set_xlabel('Episode')
                axes[2].set_ylabel('Active Cars')
                axes[2].set_title('Average Number of Active Cars Per Episode', fontweight='bold')
